...
export class signUp {
  signUpNavigation() {
    cy.visitnaukri(`/${Cypress.env("registration_end_point")}`);
    cy.title().should(
      "eq",
      "Register on Naukri.com: Apply to Millions of Jobs Online"
    );
  }
  ...
  signupelementvisibility() {
    Object.keys(register).forEach((key) => {
      if (
        (key != "fullnameerrorlabel") & (key != "emailerrorlabel") &&
        key != "passworderrorlabel" &&
        key != "mobileerrorlabel" &&
        key != "currentcityerrorlabel" &&
        key != "currentcountryerrorlabel" &&
        key != "outsideindiacheckbox" &&
        key != "outsideindialabel" &&
        key != "currentcitylabel" &&
        key != "currentcitytxt" &&
        key != "outsidecityindiatxt" &&
        key != "currentcountrytxt" &&
